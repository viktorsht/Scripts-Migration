...
# ── SSH helpers ────────────────────────────────────────────────────────────────
# ssh_run  → executa e exibe saída (stderr suprimido para evitar warnings)
ssh_run() {
...
# ssh_cap → captura saída para variável (limpa, sem warnings)
ssh_cap() {
  local host="$1" user="$2"; shift 2
  ssh -i "$SSH_KEY" \
      -o StrictHostKeyChecking=no \
      ...
      -o PasswordAuthentication=no \
      -o LogLevel=ERROR \
      "${user}@${host}" "$@" 2>/dev/null | grep -v "^Warning" | grep -v "^$" || true
}
...
R_STATUS=$(ssh_cap "$R_HOST" "$R_USER" \
...
M_CSTATUS=$(ssh_cap "$M_HOST" "$M_USER" \
...
EXISTS_USER=$(ssh_cap "$M_HOST" "$M_USER" \
  "docker exec ${M_DBC} mysql -uroot -p${M_PASS} --silent \
...
MASTER_STATUS=$(ssh_cap "$M_HOST" "$M_USER" \
  "docker exec ${M_DBC} mysql -uroot -p${M_PASS} --silent \
...
DUMP_SIZE=$(ssh_cap "$M_HOST" "$M_USER" "du -sh ${DUMP_REMOTE} | cut -f1")
success "Dump gerado: ${DUMP_REMOTE} (${DUMP_SIZE})"

# # =============================================================================
# #  FASE 6a — BLOQUEAR ESCRITA NA ORIGEM (CUTOVER INICIA AQUI)
# # =============================================================================
# step "FASE 6a/11 — Bloqueando escrita na origem (cutover)"

# info "Parando API na origem (evita novas escritas)..."
# ssh_run "$M_HOST" "$M_USER" \
#   "docker stop ${M_APIC} 2>/dev/null || true" \
#   | while read -r l; do dim "$l"; done

# success "API da origem parada."

# info "Travando banco para consistência final..."
...
# success "Escrita bloqueada na origem (LOCK ativo)."

# info "Aguardando 3s para garantir flush final..."
# sleep 3

# =============================================================================
#  FASE TESTE — MATANDO O SERVIÇO DA ORIGEM PARA EVITAR DADOS SEM SINCRONIA.
# =============================================================================

step "FASE TESTE — Desativando sistema na origem (FORÇADO)"
step "FASE TESTE — MATANDO O SERVIÇO DA ORIGEM PARA EVITAR DADOS SEM SINCRONIA."

info "Desabilitando restart policy da API e do banco..."
ssh_run "$M_HOST" "$M_USER" "
  docker update --restart=no ${M_APIC} 2>/dev/null || true
  docker update --restart=no ${M_DBC} 2>/dev/null || true
"

info "Parando e removendo API na origem..."
ssh_run "$M_HOST" "$M_USER" "
  docker stop ${M_APIC} 2>/dev/null || true
  docker rm -f ${M_APIC} 2>/dev/null || true
"

success "API '${M_APIC}' removida da origem."

info "Parando e removendo banco na origem..."
ssh_run "$M_HOST" "$M_USER" "
  docker stop ${M_DBC} 2>/dev/null || true
  docker rm -f ${M_DBC} 2>/dev/null || true
"
...
# =============================================================================
#  FASE 8 — CONFIGURAR REPLICAÇÃO MySQL (REMOVIDO)
# =============================================================================
...
# SLAVE_STATUS=$(ssh_cap "$R_HOST" "$R_USER" \
...
for i in $(seq 1 24); do
  HTTP=$(curl -s -o /dev/null -w "%{http_code}" \
    --connect-timeout 3 \
    "http://${R_HOST}:${R_APIPORT}/health" 2>/dev/null || echo "000")
  if [[ "$HTTP" =~ ^[2345] ]]; then
    API_OK=true
...
echo -e "${GREEN}${BOLD}║   Migração para ${R_HOST} concluída com sucesso!                        ║${NC}"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════╝${NC}"
echo ""
cat "$SUMMARY_FILE"
echo ""
